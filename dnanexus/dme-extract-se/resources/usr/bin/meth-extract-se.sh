...
if [ $# -lt 4 ] || [ $# -gt 4 ]; then
    echo "usage v1: meth-extract-se.sh <index.tgz> <bismark.bam> <ncpus> [--scorched_earth]"
...
set -x
mkdir -p output/
bismark_methylation_extractor --multicore $ncores --single-end -s --comprehensive -output output/ ${alignments_file}

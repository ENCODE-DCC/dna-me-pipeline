...
def bismark_full_extract(target_root, alignments, ncores):
    ...
    # NOTE: Better to use sam and let extractor use more threads, but this takes up precious storage
    (alignments, ncores) = bam_or_sam(biorep_bam, biorep_bam_dxlink, uncompress_bam, target_root)
...
@dxpy.entry_point("extract")
def extract(biorep_bam_dxlink, dme_ix_dxlink, uncompress_bam, target_root, qc_metrics, props):
    # subjob runs bismark_methylation_extractor on mem1_hdd2_x32

    print "* extract(): Retrieve merged bam and index..."
    ...
    print "* extract(): Uncompress index..."
    ...
    print "* extract(): Storing extraction results..."
    split_report_dxfile = dxpy.upload_local_file(target_root+'_splitting_report.txt')
    cx_report_dxfile = dxpy.upload_local_file('output/'+target_root+'.CX_report.txt')
    bedgraph_gz_dxfile = dxpy.upload_local_file('output/'+target_root+'.bedGraph.gz')
...
def bismark_coverage(target_root, CpG_context, CHG_context, CHH_context, gzip=True, cleanup=False):
    ...
    cmd = 'bismark2bedGraph --CX_context --ample_mem  --zero_based --dir output/ -output '
    cmd += '%s %s %s %s' % (bedGraph, CpG_context, CHG_context, CHG_context)
    run_cmd(cmd)
    run_cmd('ls -l output/')
...
def bedmethyl_io(cx_report_dxlink, chrom_sizes_dxlink, target_root, qc_metrics, props):
    ...
    dxpy.download_dxfile(cx_report_dxlink, cx_report)
    run_cmd('mkdir -p output/')
    run_cmd('mv %s output/%s' % (cx_report,cx_report) ) 
...
def main(bam_set, map_report_set, dme_ix, uncompress_bam=True, nthreads=8):
    ...
    #extract_job = dxpy.new_dxjob(fn_input=inp, fn_name="extract")
    extract_job = dxpy.new_dxjob(inp, "extractor")
    print "* Kicked off extract() and waiting..."

    print "* Calling coverage()..."
    inp = {
        'CpG_context_dxlink': extract_job.get_output_ref("CpG_context_dxlink"),
        'CHG_context_dxlink': extract_job.get_output_ref("CHG_context_dxlink"),
        'CHH_context_dxlink': extract_job.get_output_ref("CHH_context_dxlink"),
        'dme_ix_dxlink':      dme_ix,
        'target_root':        target_root,
    ...
    inp = {
        ...
        'chrom_sizes_dxlink': extract_job.get_output_ref("chrom_sizes_dxlink"),
        'target_root': target_root,
        'qc_metrics': extract_job.get_output_ref("qc_metrics"),
        'props': props,
    }

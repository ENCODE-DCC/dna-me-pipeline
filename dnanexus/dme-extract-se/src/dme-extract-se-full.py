...
# NOTE: extract() is not used, but would allow running merge on main instance and onlt extract on x32 instance
...
def extract(biorep_bam_dxlink, dme_ix_dxlink, uncompress_bam, target_root, qc_metrics, props):
    ...
    # NOTE: Better to use sam and let extractor use more threads, but this takes up precious storage
    (alignments, ncores) = bam_or_sam(biorep_bam, uncompress_bam, target_root, dxpy.describe(biorep_bam_dxlink)['size'])
